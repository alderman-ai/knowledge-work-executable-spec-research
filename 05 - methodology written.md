...
- **Definitions, created before gate 06 fires** (the harness registers `.claude/agents/*.md` at the vault
  ...
  session — FOLDER DIRECTORY, `Skills/`). Three, all **`model: opus`, `effort: high`**:
...
- [ ] **WP6 · Round trip.** *(orchestrator runs the program; five auditors judge in parallel)* For each
      ...
      naming every visible difference region or stating the list is empty. A non-empty list is the M4
      loop: repair the translator, re-import, redeploy, re-judge. Finally `git -C <codebase> diff
...
| Agent definitions | `<vault root>/.claude/agents/webpage-{builder,auditor,skill-runner}.md` | this Methodology's dispatches |
