...
- [x] **WP0 · Baseline and tooling.** *(orchestrator, then builder)* Record `<codebase>` HEAD SHA and a
...
- [x] **WP1 · Canon inventory and contradiction sweep.** *(five auditors in parallel, then one auditor)*
...
- [x] **WP2 · Schema and lexicon.** *(builder, then auditor adversarially, then builder repairs)* Builder
...
- [x] **WP3 · The translator.** *(builder; one dispatch per command)* In `Working/translator/`:
...
- [x] **WP4 · Export the five pages.** *(orchestrator runs the program; five auditors reconcile in
...
- [x] **WP5 · Intake template and block library.** *(builder, then auditor)* Builder writes
...
- [x] **WP6 · Round trip.** *(orchestrator runs the program; five auditors judge in parallel)* For each
...
- [x] **WP7 · New page, one shot, and determinism.** *(builder fills the template; skill-runner
...
- [x] **WP8 · The three skills.** *(builder; authored right after WP3, run at WP7 and WP9)* Under the
...
- [x] **WP9 · Fresh-session runs of the remaining two skills.** *(two skill-runners, sequential)* Each
...
- [x] **WP10 · Close.** *(orchestrator)* Diffs: `git -C <codebase> diff --stat <WP0 SHA> -- components/`
...
|---|---|---|---|
| Production SHA vs WP0 HEAD SHA | Vercel production deployment | `CLAUDE.md` treats the codebase as "the local codebase of the deployed site" | H4 fired mid-run: `main` moved 9e0e9ef→78f20ed (operator cleanup, render-neutral); operator ruled re-baseline 2026-08-25. Production SHA not verified; per D8 the 5.2 baseline was a preview of the unmodified 78f20ed, so production never entered the comparison. |
...
**D11 — The intake template uses the operator's human-friendly form; the importer normalises it to
the canonical form.** *(Operator ruling 2026-08-25, mid-execution, after reviewing the WIP intake sketch
in parallel: "I want these more human friendly intake format to be part of the design." Feedback and
directional draft: `Working/Operator Feedback on WIP UX.md`, `Working/WIP-intake-ux-sketch REWRITE.md`.)*
Principle: a block's description is a fixed fact, never a choice; every variant is a closed choice
selected by bolding one option inside a `>[!Variants]` callout (default pre-bolded); copy goes in fenced
slots; the template carries zero instructions. Applied as: the intake template (WP5, DoD 3.1) and block
library (3.2) are authored in this **intake form**; the translator's `validate`/`import` (WP3) accept the
intake form and normalise it deterministically to the schema's **canonical form** — the form `export`
emits — before validating; the round trip (5.2) and exports (2.x) stay on the canonical form. Two
bolded options, or none, is a rejection with a named reason. Two items from the draft are carried to the
handover as follow-ons rather than built here: a mermaid page-flow canvas (a new deliverable) and NEW
asset upload (a write outside the new page's route directory, which DoD 4.3 forbids this run); the
template lists existing assets exhaustively instead. **Reverse this and** the template reverts to the
canonical form and the importer loses its normalisation step; nothing else moves.
...
**Ticked 2026-08-25, session `427a4720-66b6-4eae-a372-a2118f883bd0`.** Every box's evidence is mapped in
`Working/evidence-index.md` (one row per box → artifact) and narrated in `Working/run-log.md`; receipts in
`Working/receipts/WP0–WP10`. Three boxes rest on in-session operator rulings recorded in the run log: 2.2/3.3
(capture evidence substituted by the operator's direct read — display unavailable) and 1.2 (canon-visible
values only). Preview URLs: round-trip https://alderman-ccoomtd5t-alex-alderman-s-projects.vercel.app ·
baseline https://alderman-ehemczca3-alex-alderman-s-projects.vercel.app · one-shot demo
https://alderman-izupmrto2-alex-alderman-s-projects.vercel.app/template-demo .
...
- [x] **1.1 A written schema and lexicon exists** at a registered path in `Deliverables/`, and it can express every block appearing on the five in-scope pages. *Evidence:* the document itself, plus a coverage table with one row per block name giving the component it renders to and the props it sets.
- [x] **1.2 The lexicon names nothing that is not canon.** Every block name traces to something visibly rendered on alderman.ai; no name exists for a component that is unused, superseded, or `/dev`-only (D1). *Evidence:* the coverage table's canon column, each row citing the in-scope page and location where that block is visible.
- [x] **1.3 The lexicon introduces no new web components** (A1). *Evidence:* every component named in the coverage table already exists in `components/` at the commit the work started from, shown by a diff of that directory containing no additions.
- [x] **1.4 The colour rule is specified as a total function** — given any body string it yields exactly one segmentation, with its escape hatches fully specified so no input is undefined. *Evidence:* the rule stated in the schema, plus a worked-example table that includes at least one case the automatic rule cannot produce and which therefore requires an explicit marker.
- [x] **1.5 Every part of a page the schema does *not* represent is named explicitly** as code-owned, rather than left silent. *Evidence:* a code-owned inventory in the schema listing each such item and the page it belongs to.

**2 · Export — a URL becomes a legible MD file**

- [x] **2.1 Each in-scope page exports to an MD file** conforming to the schema. *Evidence:* five files at a registered path, one per page.
- [x] **2.2 Each exported file is legible in the Obsidian editing UI** — headings render as headings, lists as lists, no raw schema machinery showing as noise, no broken markup. *Evidence:* a rendered capture of each of the five files as Obsidian displays them.
- [x] **2.3 Each exported file carries the whole page.** No block visible on the live page is missing from its MD, and the MD invents nothing the live page does not show. *Evidence:* a per-page block-by-block reconciliation against the live page.

**3 · The intake template and the block library**

- [x] **3.1 An intake template for a new page exists**, carrying a default set of sections and components ready to fill in. *Evidence:* the template file at a registered path.
- [x] **3.2 A separate block library exists**, holding every block the lexicon defines, each one copy-pasteable into the template unmodified. *Evidence:* the library file, plus a two-way check that no lexicon block is missing from the library and no library block is absent from the lexicon.
- [x] **3.3 Template and library are legible in Obsidian** on the same terms as 2.2, and the library states for each block what it looks like and when to reach for it. *Evidence:* rendered captures of both files.

**4 · Import — a completed template becomes a deployed page in one shot**

- [x] **4.1 A completed intake template becomes a working page at a Vercel preview URL** (A2), from a single invocation with no human intervention between starting it and the URL existing. *Evidence:* the preview URL, plus a run record showing one invocation and no intervening manual step.
- [x] **4.2 The generated page passes the codebase's own checks.** *Evidence:* clean output from `npm run typecheck`, `npm run lint` and `npm run build` in the run record.
- [x] **4.3 The generated page adds no new components and changes no existing one** (A1). *Evidence:* a codebase diff for the run showing additions confined to the new page's own route directory.
- [x] **4.4 The generated page renders correctly in the browser** — every block present, in template order, in the right register. *Evidence:* a capture of the preview URL alongside the template it came from.

**5 · Determinism**

- [x] **5.1 The same MD produces byte-identical code on every run.** *Evidence:* two independent runs over the same input, with matching hashes for every generated file.
- [x] **5.2 The round trip is faithful.** For each in-scope page, exporting it and then re-importing that MD produces a page visually indistinguishable from the live page. *Evidence:* a per-page comparison record naming any difference found, with an empty difference list for all five.
- [x] **5.3 No verification step modified a live page.** The round trip renders to preview only; the source of the five in-scope pages is untouched. *Evidence:* a clean diff of the five live page files across the whole run.
- [x] **5.4 Ambiguity is impossible rather than merely absent.** No schema construct has two valid code outputs, and any MD the schema cannot represent is rejected with a named reason instead of being guessed at. *Evidence:* a record of rejected-input cases showing the reason given for each.

**6 · The skills**

- [x] **6.1 One skill exists per capability named in the Problem** — export a URL to MD, produce the intake template and library, and convert a completed template into a deployed page. *Evidence:* the skill files in `Skills/`.
- [x] **6.2 Each skill runs end to end from a fresh session** with nothing but its own instructions and the artifacts it names — no reliance on context from the session that wrote it. *Evidence:* a run record per skill from a session that did not author it.

**7 · Prohibitions and the record the operator asked for**

- [x] **7.1 The brand and design system are unchanged.** *Evidence:* a clean diff of `design-system/` across the whole project (D2).
- [x] **7.2 The contradiction list exists** (A3), naming each place the codebase, `DESIGN.md` and the cards disagree, and which side canon supports. *Evidence:* the list at a registered path in `Working/`, each entry citing the disagreeing artifacts and its canon verdict.
- [x] **7.3 Contradictions the live site could not settle were put to the operator in one batch** (D3), not one at a time. *Evidence:* the single rendered comparison page, or an explicit note that no contradiction required the operator.
